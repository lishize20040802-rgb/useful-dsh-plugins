...
test('default image inbox follows DSH_HOME including relative and tilde paths', () => {
  const saved = process.env.DSH_HOME
  try {
    const cases = [
      [undefined, join(homedir(), '.dsh')],
      [' \t ', join(homedir(), '.dsh')],
      [join(tmpdir(), 'vision-home-test'), join(tmpdir(), 'vision-home-test')],
    ...
    for (const [declared, expected] of cases) {
      ...
      assert.equal(normalizeConfig({}).inboxDir, join(expected, 'vision-inbox'))
      assert.equal(normalizeConfig({ inboxDir: ' \t ' }).inboxDir, join(expected, 'vision-inbox'))
    }
  } finally {
...
test('an explicitly configured image inbox remains unchanged', () => {
  const declared = join(tmpdir(), 'synthetic-explicit-image-directory')
  assert.equal(normalizeConfig({ inboxDir: declared }).inboxDir, declared)
})
