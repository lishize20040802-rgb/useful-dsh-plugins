...
| `uploadDir` | `$DSH_HOME/uploads` | Directory files are saved to (content-addressed as `<sha256-prefix>-<name>`). Point it at a directory visible to the agent's workspace when the default is not. |
...
- Scanned PDFs and other binary formats upload fine, but their content readability depends on a suitable reader plugin (e.g. `dsh-plugin-doc-companion`, which supplies document reading and optional scanned-page transcription).
...
```sh
dsh plugin --profile web remove dsh-upload-button
```
