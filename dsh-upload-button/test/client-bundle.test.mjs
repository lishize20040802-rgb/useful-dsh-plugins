...
test('client bundle is a ModuleLoader factory envelope', () => {
  ...
  assert.ok(!src.includes('provide("chatFileMentions"'), 'the official deliverables plugin owns chatFileMentions; providing it is a boot failure')
  assert.ok(src.includes('openFile'), 'file open must go through the framework-supplied opener')
...
test('client bundle attaches files at send time via the session prompt facade', () => {
  ...
  assert.match(src, /binding(?:\?\.)?\(/, 'session binding resolution must survive bundling')
})
...
test('client bundle shadows the user bubble with words-only + floating cards', () => {
  ...
  assert.ok(src.includes('renderMessageImages'), 'images must render through the framework-supplied gallery seat')
  assert.ok(!src.includes('require("@deepseek-ai/dsh-client-ui-attachment")'), 'the plugin must not compose a gallery of its own')
})

test('declaration outputs accompany the bundles', () => {
  assert.ok(!existsSync(fileURLToPath(new URL('../lib/client.js.map', import.meta.url))), 'release excludes source maps')
