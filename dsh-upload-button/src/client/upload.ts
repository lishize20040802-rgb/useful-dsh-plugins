...
const wrappedFaces = new Map<SessionFace, { original: SessionFace['prompt']; wrapper: SessionFace['prompt']; active: boolean }>()

/** Restore only our own hook and let wrappers installed later remain intact. */
export function disposeSendAttachments(): void {
...
export function installSendAttachment(sessions: any, sessionId: SessionId): void {
  ...
  const wrapper = (content: unknown, mode: unknown) => {
    ...
    return Promise.resolve(original.call(session, parts, mode)).then((result: any) => {
