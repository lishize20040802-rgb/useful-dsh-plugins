...
固定 + 条件性：上述提示词段落恒定（约 0.5 K token）。`doc_current` / `doc_page` / `doc_search` 的结果长度可变——整页提取文本可达数千 token；检索片段受 `searchMaxHits` × 约 300 字符上限约束。扫描页转写每页至多一次，并缓存在内存中。
...
- **PDF 视觉转写有成本与延迟**——PDF 阅读一律走视觉模型转写（每页首次约 3–5 秒，结果按页缓存于内存，重启后首次访问重新转写）；公式复杂的页面转写可能不完美，必要时可参考 `image_path` 快照。
...
```sh
npm install --legacy-peer-deps   # peer 依赖经 harness 安装解析（D:\harness\node_modules）
