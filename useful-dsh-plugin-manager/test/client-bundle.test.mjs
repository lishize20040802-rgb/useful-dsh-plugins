...
test('client bundle is a ModuleLoader factory envelope', () => {
  ...
  assert.ok(src.includes('/host/update'), 'separate host update controls must survive bundling')
  assert.ok(src.includes('confirm: true'), 'the host update requires an explicit user selection')
...
test('release has no source map or sourceMappingURL', () => {
  assert.equal(existsSync(fileURLToPath(new URL('../lib/client.js.map', import.meta.url))), false)
  assert.doesNotMatch(readFileSync(fileURLToPath(new URL('../lib/client.js', import.meta.url)), 'utf8'), /sourceMappingURL/)
})
